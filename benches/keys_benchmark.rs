...
use redis_proto::keys::{key_interact, KeyOps};
use redis_proto::ops::{op_interact, translate};
use redis_proto::types::{RedisValueRef, ReturnValue, State, StateStore};
use std::sync::Arc;
...
fn bench_translate(c: &mut Criterion) {
    ...
    group.bench_function("translate", |b| {
        b.iter(|| translate(black_box(value.clone()), black_box(state_store.clone())));
    });
    group.finish();
}
    ...
    c.bench_function("full_life_cycle", |b| {
        b.iter(|| async {
            ...
            let res = decoder
                ...
                .expect("parsing to work")
                .unwrap();
            let op = translate(black_box(res), black_box(state_store.clone())).unwrap();
